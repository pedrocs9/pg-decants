...
export default function NotesPage() {
  ...
  async function handleDelete(id: number) {
    if (confirm('¿Eliminar esta nota olfativa?')) {
      await deleteNote(id);
      refresh();
    }
  }
  ...
  return (
    <div className="max-w-xl">
      <h1 className="font-display italic text-3xl text-brand-text-dark mb-8">Notas Olfativas</h1>
      ...
      <form onSubmit={handleCreate} className="bg-brand-white border border-brand-beige-line p-6 mb-6 flex gap-3 items-end">
        <div className="flex-1">
          ...
          <input
            ...
            placeholder="Ej: Bergamota, Vainilla, Oud..."
      ...
      <div className="bg-brand-white border border-brand-beige-line">
        {notes.map((n) => (
          <div key={n.id} className="flex items-center justify-between px-4 py-3 border-b border-brand-beige-line last:border-0">
            <span className="text-brand-text-dark">{n.name}</span>
            <button onClick={() => handleDelete(n.id)} className="text-red-600 text-sm hover:underline cursor-pointer">
              Eliminar
            </button>
          </div>
        ))}
        {notes.length === 0 && <p className="text-center py-8 text-brand-text-muted">Sin notas todavía.</p>}
      </div>
    </div>
  );
}