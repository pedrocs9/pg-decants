...
import { getFamilies, createFamily, deleteFamily } from './actions';
...
export default function FamiliesPage() {
  ...
  async function handleDelete(id: number) {
    if (confirm('¿Eliminar esta familia olfativa?')) {
  ...
  return (
    <div className="max-w-xl">
      <h1 className="font-display italic text-3xl text-brand-text-dark mb-8">Familias Olfativas</h1>
      ...
      <form onSubmit={handleCreate} className="bg-brand-white border border-brand-beige-line p-6 mb-6 flex gap-3 items-end">
        <div className="flex-1">
          ...
          <input
            ...
            placeholder="Ej: Amaderado, Floral, Cítrico..."
      ...
      <div className="bg-brand-white border border-brand-beige-line">
        {families.map((f) => (
          <div key={f.id} className="flex items-center justify-between px-4 py-3 border-b border-brand-beige-line last:border-0">
            <span className="text-brand-text-dark">{f.name}</span>
            <button onClick={() => handleDelete(f.id)} className="text-red-600 text-sm hover:underline cursor-pointer">
              Eliminar
            </button>
          </div>
        ))}
        {families.length === 0 && <p className="text-center py-8 text-brand-text-muted">Sin familias todavía.</p>}
      </div>
    </div>
  );
}